...
// 
/**\class SUSYSkimToTreeTFS SUSYSkimToTreeTFS.cc TopAnalysis/SUSYSkimToTreeTFS/src/SUSYSkimToTreeTFS.cc
...
//#include "TopAnalysis/TopTreeProducer/interface/TriggerBitChecker.h"
...
//  std::vector<double> *T_Gen_polWeights;
  ...
  std::vector<float> *T_Muon_dxyBestTrack;
  std::vector<float> *T_Muon_dzBestTrack;
...
//		cout<<"Particle "<<id <<" with status "<<st<<"and no mother"<<endl;
  ...
  T_Muon_dxyBestTrack = new std::vector<float>;
  T_Muon_dzBestTrack = new std::vector<float>;
  ...
  for (size_t k = 0; k < selected_muons.size(); ++k) {
	...
	dZ	       =  selected_muons[k].globalTrack()->dz(vtxs[0].position());
        tightMuon      =  selected_muons[k].isTightMuon(vtxs[0]);
      }
      
    
    }

    T_Muon_IsTightMuon->push_back(tightMuon);
    ...
    T_Muon_dxyBestTrack->push_back(selected_muons[k].muonBestTrack()->dxy(vtxs[0].position()));
    T_Muon_dzBestTrack->push_back(selected_muons[k].muonBestTrack()->dz(vtxs[0].position()));
...
//  delete T_Gen_polWeights;
  ...
  delete T_Muon_dxyBestTrack;
  delete T_Muon_dzBestTrack;
...
//  Tree->Branch("T_Gen_polWeights", "std::vector<double>", &T_Gen_polWeights);
  ...
  Tree->Branch("T_Muon_dxyBestTrack","std::vector<float>", &T_Muon_dxyBestTrack);
  Tree->Branch("T_Muon_dzBestTrack","std::vector<float>", &T_Muon_dzBestTrack);
  ...
  */  
  ...
  Tree->Branch("T_Elec_sumPUPu", "std::vector<float>", &T_Elec_sumPUPt);
